...
test("سقف عمر الذاكرة يُقصَّر ولا يُمدَّد من الواجهة", () => {
  // ★ لماذا ٣٠٠ لا ٣٦٠٠ ★ عمر النسخة المخبّأة نافذةُ قراءة **بعد سحب الصلاحية**:
  //   مفتاح الذاكرة يتضمّن قدرة الحساسية، فسحب الملكية يُبطل النسخة فورًا، لكنّ
  //   سحب صلاحية في موديول مصدر (operations.view مثلًا) لا يغيّر المفتاح. سقف
  //   ٣٦٠٠ كان يجعل تلك النافذة ساعةً **يختارها المتصل نفسه** بتمرير ttl_seconds.
  const body = funcBody("mgmt_norm_filters");
  const m = body.match(/least\(greatest\(coalesce\([\s\S]{0,120}?\)\s*,\s*0\)\s*,\s*(\d+)\)/);
  assert.ok(m, "ttl_seconds بلا سقف صريح — المتصل يجمّد الأرقام كما يشاء");
  const ceiling = Number(m[1]);
  assert.ok(ceiling <= 300,
    `سقف عمر الذاكرة ${ceiling}ث — المتصل يمدّد نافذة القراءة بعد سحب صلاحيته`);
  assert.match(body, /coalesce\(nullif\(btrim\(coalesce\(f->>'ttl_seconds'[\s\S]{0,40}?::int, 300\)/,
    "الافتراض ليس ٣٠٠ ثانية");
});
