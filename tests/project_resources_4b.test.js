...
test("إصلاحات البوابة النهائية 4B (المراجعة العدائية الكاملة)", () => {
  const conf = funcBody("resource_booking_conflicts");
  // #2: سعة المعدة تضمّ حجوزات العهدة النشطة (لا تجاوز فوق ما حجزته العهدة)
  assert.match(conf, /v_used := v_used \+ coalesce\(\(select sum\(rr\.quantity\) from public\.custody_inventory_reservations rr/i, "السعة لا تضمّ حجوزات العهدة");
  const create = funcBody("resource_booking_create");
  // #3: فحص انتماء المهمة/الجلسة حتى عند غياب project_id
  assert.match(create, /if v_task is not null then[\s\S]*?elsif not exists[\s\S]*?pc_can_read_project\(t\.project_id\)/i, "لا فحص ملكية المهمة عند غياب project_id");
  assert.match(create, /if v_shoot is not null then[\s\S]*?pc_can_read_project\(s\.project_id\)/i, "لا فحص ملكية الجلسة عند غياب project_id");
  // #4: الإنشاء بحالة confirmed يتطلب confirm_booking
  assert.match(create, /v_status = 'confirmed' and not public\.res_can\('resources\.confirm_booking'\)/i, "الإنشاء بحالة confirmed لا يتطلب صلاحية التأكيد");
  // #5: prorate planned_hours بنسبة أيام العمل، مع coalesce→1 لنطاق بلا أيام عمل (لا إسقاط null)
  assert.match(funcBody("employee_workload_core"), /coalesce\(public\.working_days_between\(greatest\(t\.start_date, p_from\), least\(t\.due_date, p_to\)\)::numeric[\s\S]*?nullif\(public\.working_days_between\(t\.start_date, t\.due_date\), 0\), 1\)/i, "planned_hours غير مقسّمة/غير محميّة من الإسقاط");
  // #6: confirm بالهوية (type+booking_id) لا بالعدد — تعارض حاجب جديد لا يمرّ بتجاوز قديم
  assert.match(funcBody("resource_booking_confirm"), /b\.overridden_conflicts @> jsonb_build_array\(jsonb_build_object\('conflict_type', c\.conflict_type, 'conflicting_booking_id', c\.conflicting_booking_id\)\)/i, "confirm يقارن بالعدد لا بالهوية");
  assert.doesNotMatch(funcBody("resource_booking_confirm"), /v_hard > \(select count/i, "confirm ما زال يقارن بالعدد");
});
