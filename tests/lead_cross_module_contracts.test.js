...
test("★ المرجع الماليّ لا يُعيد مبلغًا إطلاقًا — عرّافة السعر مغلقة ★", () => {
  // العقد يسمح لهذه الوحدة بقراءة **وجود** الذمّة ومرجعها وحالتها واستحقاقها
  // وحالة تحصيل عامّة — لا قيمتها. مندوب يقرأ المبلغ التعاقديّ يستنتج قيمة
  // الصفقة، ومن المقارنة بين صفقتين تُشتقّ الأرضية والهامش. الحجب في المصدر.
  const fn = funcBody("lsr_finance_reference");
  for (const k of ["price_net", "price_gross", "vat_rate", "vat_amount",
                   "amount_net", "amount_gross", "renewal_amount_net"]) {
    assert.ok(!new RegExp(`[a-z]\\.${k}\\b`).test(fn),
      `★ تسريب قيمة ★ المرجع الماليّ يقرأ «${k}» — العقد يسمح بالمرجع والحالة والاستحقاق لا بالقيمة`);
  }
  // وما يُعاد بدلًا منها: مرجع وحالة واستحقاق وحالة تحصيل عامّة.
  for (const k of ["receivable_reference", "due_date", "collection_state",
                   "amounts_excluded_by_contract"]) {
    assert.ok(fn.includes(k), `الحقل المسموح «${k}» غائب — الحجب لا يعني الصمت`);
  }
});

test("الضريبة حقل مستقلّ حيث يُعرض مبلغ أصلًا", () => {
  // حيث يُسمح بالمبلغ (لوحة العميل: أرقامه هو) لا تُطوى الضريبة في الإجمالي.
  const fn = funcBody("lsr_dashboard_client");
...
test("POSTCHECK يحمل صفوف العقود، وكلّها قادرة على الإخفاق", () => {
  ...
  // عقد المالية صار أربعة صفوف لا صفًّا واحدًا: كتابة، نداء خارجيّ، قراءة
  // مسموحة فقط، والتفاف غير مباشر. الأخير هو ما فات حزمةً سابقة.
  for (const row of [/لا كتابة ماليّة في أيّ دالّة/, /لا نداء خارجيّ في أيّ دالّة/,
                     /قراءات ماليّة مسموحة فقط/, /لا خرق غير مباشر عبر ما يُنادى/,
                     /حالة السداد معلَنة كقراءة فقط/, /المرجع الماليّ بلا مبلغ/]) {
    assert.match(POSTCHECK, row, `POSTCHECK بلا صفّ لعقد المالية: ${row}`);
  }
