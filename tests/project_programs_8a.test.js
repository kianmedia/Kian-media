...
test("متابعة المراجعة: خطأ رقم الوحدة مترجم عند النقل، والرابط العميق للبرنامج يُحلّ", () => {
  const HIER = read("lib/portal/projectHierarchy.ts");
  assert.match(HIER, /ux_projects_parent_unit_number\|duplicate_unit_number/, "23505 من فهرس رقم الوحدة يظهر خامًا عند النقل");
  assert.match(OPS, /if \(initialTab === "program" && isMaster\) setTab\("program"\)/, "‏?tab=program لا يُحلّ بعد وصول سياق الهرمية");
});
