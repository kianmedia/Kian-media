...
test("STAB (٥) تحصينات المراجعة النهائية: نزع project_client_user_ids + ppc_read بـis_active", () => {
  assert.match(STAB, /revoke all on function public\.project_client_user_ids\(uuid\) from public, anon, authenticated/,
    "دالّة مساعِدة SECURITY DEFINER لا تزال قابلة للاستدعاء من authenticated");
  assert.match(STAB, /create policy ppc_read on public\.preproduction_comments[\s\S]{0,400}i\.is_active = true/,
    "ppc_read لا يعكس is_active (العميل يقرأ تعليقات عنصر مُعطَّل)");
  // مُحصَّنان بحيث لا يُجهضان الترحيل إن غاب الهدف
  assert.match(STAB, /if to_regprocedure\('public\.project_client_user_ids\(uuid\)'\) is not null then/, "النزع غير محروس");
  assert.match(STAB, /if to_regclass\('public\.preproduction_comments'\) is not null/, "إعادة السياسة غير محروسة");
});

test("الـRunbook يذكر متطلّب كتالوج الصلاحيات قبل دفعات المنصّة", () => {
  const rb = read("docs/PROJECT_PLATFORM_PRODUCTION_RUNBOOK.md");
  assert.match(rb, /permission_catalog_RUNME\.sql/, "الـRunbook لا يذكر كتالوج الصلاحيات");
  assert.match(rb, /5A\/6A\/8A\/8D تفشل في الـPreflight بدونه/, "لا تحذير من فشل Preflight");
});
