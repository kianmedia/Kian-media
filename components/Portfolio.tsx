...
// Category definitions — order matters for the tab bar
const CATEGORIES = [
  { key: "all",         ar: "كل الأعمال",   en: "All",          icon: "✦" },
  { key: "events",      ar: "تغطية فعاليات", en: "Events",       icon: "🎪" },
  { key: "documentary", ar: "أفلام وثائقية", en: "Documentary",  icon: "🎞️" },
...
// Real portfolio pulled from kianmedia.com — grouped by project type
const ITEMS: Item[] = [
  // Real estate
...
function Card({ item, idx, onOpen }: { item: Item; idx: number; onOpen: (yt: string) => void }) {
  ...
  return (
    <motion.button
      ...
      className="group relative block w-full text-right overflow-hidden"
      style={{ aspectRatio: "16/10", border: "1px solid rgba(227,30,36,.1)", background: "#070707", cursor: "pointer" }}
    >
      {/* Thumbnail — lazy; no iframe until click */}
      <img
        ...
        style={{ opacity: 0.5 }}
      />
      <div className="absolute inset-0 transition-opacity duration-500" style={{ background: "linear-gradient(to top, rgba(0,0,0,.95) 0%, rgba(0,0,0,.4) 50%, rgba(0,0,0,.1) 100%)" }} />
      <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500" style={{ background: "linear-gradient(to top, rgba(227,30,36,.22), transparent 60%)" }} />

      {/* Play */}
      <div className="absolute inset-0 flex items-center justify-center">
        <span className="flex items-center justify-center transition-all duration-400 group-hover:scale-110" style={{ width: "58px", height: "58px", borderRadius: "50%", background: "rgba(227,30,36,.9)", boxShadow: "0 8px 32px rgba(227,30,36,.5)", backdropFilter: "blur(2px)" }}>
      ...
      <div className="absolute bottom-0 right-0 left-0 p-5 text-right">
        <span className="f-mont inline-block mb-2" style={{ fontSize: "8px", letterSpacing: "2.5px", color: "rgba(227,30,36,.95)", textTransform: "uppercase", padding: "3px 8px", border: "1px solid rgba(227,30,36,.3)", background: "rgba(227,30,36,.08)" }}>
          {meta?.icon} {meta?.ar}
        </span>
        <h3 className="text-white font-semibold leading-snug" style={{ fontSize: "15px" }}>{item.ar}</h3>
        <p className="f-mont text-white/40 mt-1" style={{ fontSize: "9px", letterSpacing: ".5px" }}>{item.en}</p>
      </div>
...
export default function Portfolio() {
  ...
  return (
    <section id="portfolio" style={{ background: "#000", paddingTop: "112px", paddingBottom: "112px" }} className="relative overflow-hidden">
      {/* ambient glow */}
      <div className="absolute top-1/4 left-0 pointer-events-none" style={{ width: "40vw", height: "40vh", background: "radial-gradient(circle, rgba(227,30,36,.05), transparent 70%)" }} />

      <div className="max-w-7xl mx-auto px-6 lg:px-12 relative z-10">
        {/* Heading */}
        <div className="mb-10 text-center" data-reveal>
          <div className="sec-label mb-5">أعمالنا</div>
          <h2 className="sec-title">
            جزء من <span className="r">أعمالنا</span> في التصوير والإنتاج
          </h2>
          <p className="f-mont text-white/30 mt-4" style={{ fontSize: "11px", letterSpacing: "2px" }}>
            A PART OF OUR WORK IN FILMING, PRODUCTION &amp; DIRECTING
          </p>
        </div>

        {/* Category tabs — with per-category counts */}
        <div className="flex flex-wrap gap-2 justify-center mb-12" data-reveal>
          {CATEGORIES.map((c) => {
            ...
            return (
              <button
                ...
                className="f-mont flex items-center gap-2 transition-all duration-300"
                style={{
                  fontSize: "11px", letterSpacing: "1.5px", padding: "9px 16px",
                  border: "1px solid " + (on ? "#E31E24" : "rgba(255,255,255,.12)"),
                  background: on ? "rgba(227,30,36,.14)" : "transparent",
              ...
              >
                ...
                {c.ar}
                <span className="f-mont" style={{ fontSize: "9px", opacity: 0.6, background: on ? "rgba(227,30,36,.4)" : "rgba(255,255,255,.1)", padding: "1px 6px", borderRadius: "10px" }}>{n}</span>
              </button>
            );
          })}
        </div>
        ...
        <AnimatePresence mode="wait">
          ...
          >
            <span className="f-mont text-white/35" style={{ fontSize: "10px", letterSpacing: "3px", textTransform: "uppercase" }}>
              {CATEGORIES.find((c) => c.key === active)?.en} — {shown.length} {shown.length === 1 ? "PROJECT" : "PROJECTS"}
            </span>
          </motion.div>
        </AnimatePresence>
        ...
        <motion.div layout className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          <AnimatePresence mode="popLayout">
            {shown.map((item, idx) => (
              <Card key={item.id} item={item} idx={idx} onOpen={setOpen} />
            ))}
          </AnimatePresence>
        </motion.div>
        ...
        <div className="text-center mt-14" data-reveal>
          <a href="https://www.youtube.com/@kianalebtikar" target="_blank" rel="noopener noreferrer" className="btn-ghost inline-flex items-center gap-2">
            ...
            شاهد جميع أعمالنا على يوتيوب
          </a>
        </div>
      </div>

      {/* Modal — YouTube loads only here, on click */}
      <AnimatePresence>
        {open && (
          <motion.div
            initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            onClick={() => setOpen(null)}
            style={{ position: "fixed", inset: 0, zIndex: 100, background: "rgba(0,0,0,.93)", display: "flex", alignItems: "center", justifyContent: "center", padding: "20px" }}
          >
            <motion.div
            ...
            >
              <button onClick={() => setOpen(null)} className="f-mont" style={{ display: "block", marginRight: "auto", marginBottom: "12px", background: "none", border: "none", color: "rgba(255,255,255,.6)", fontSize: "13px", letterSpacing: "2px", cursor: "pointer" }}>
                ✕ إغلاق
              </button>
              <div className="yt" style={{ border: "1px solid rgba(227,30,36,.3)", boxShadow: "0 28px 80px rgba(227,30,36,.2)" }}>
                <iframe src={`https://www.youtube.com/embed/${open}?autoplay=1&rel=0&controls=1`} title="Kian Media" allowFullScreen allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" />
              </div>
            </motion.div>
          </motion.div>
        )}
