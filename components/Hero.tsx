...
export default function Hero() {
  ...
  const wa = "https://wa.me/966503422999?text=" + encodeURIComponent("السلام عليكم، أود طلب عرض إنتاج إعلامي من كيان ميديا");
  ...
  return (
    <section className="relative min-h-screen w-full flex items-center justify-center overflow-hidden" style={{ background: "#050505" }}>
      ...
      {/* Center content — visible by default, animation is enhancement */}
      <div className="relative z-10 text-center px-6 max-w-6xl mx-auto py-32" data-reveal>
        ...
        {/* English headline (editorial serif) */}
        <motion.h1
        ...
        <motion.h2
          ...
          className="f-arabic text-white/80 mb-8"
          style={{ fontSize: "clamp(18px, 2.4vw, 28px)", fontWeight: 500, lineHeight: 1.6 }}
        >
          إنتاج إعلامي سينمائي للجهات التي لا تقبل الظهور العادي
        </motion.h2>
        ...
        {/* Stats */}
        <motion.div variants={f(0.82)} initial="hidden" animate="show" className="grid grid-cols-4 gap-px max-w-xl mx-auto" style={{ background: "rgba(227,30,36,0.15)", border: "1px solid rgba(227,30,36,0.15)" }}>
          {STATS.map((s) => (
            <div key={s.l} className="text-center py-5" style={{ background: "rgba(0,0,0,0.88)" }}>
              <div className="f-display" style={{ fontSize: "clamp(22px,3vw,32px)", color: "#fff", lineHeight: 1 }}>{s.n}</div>
              <div className="f-sans mt-1.5" style={{ fontSize: "8px", letterSpacing: "2.5px", color: "rgba(255,255,255,0.4)", textTransform: "uppercase" }}>{s.l}</div>
            </div>
          ))}
        </motion.div>
        ...
        >
          ...
          <span className="f-sans" style={{ fontSize: "11px", letterSpacing: "3px", textTransform: "uppercase" }}>Watch our Showreel</span>
        </motion.button>
        ...
        {reel && (
          ...
          >
            <div onClick={(e) => e.stopPropagation()} style={{ width: "100%", maxWidth: "1000px" }}>
              <button onClick={() => setReel(false)} className="f-sans" style={{ display: "block", marginRight: "auto", marginBottom: "16px", background: "none", border: "none", color: "rgba(255,255,255,0.7)", fontSize: "13px", letterSpacing: "2px", cursor: "pointer" }}>✕ CLOSE</button>
              <div className="yt" style={{ border: "1px solid rgba(227,30,36,0.3)", boxShadow: "0 30px 100px rgba(227,30,36,0.18)" }}>
                <iframe src="https://www.youtube.com/embed/eG7K22u6xEU?autoplay=1&rel=0" title="Kian Media Showreel" allowFullScreen allow="autoplay; encrypted-media; picture-in-picture" />
              </div>
            </div>
          </motion.div>
        )}
