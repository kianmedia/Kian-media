"use client";
// ════════════════════════════════════════════════════════════════════════════
// OpsAtoms — القطع المشتركة لمركز التشغيل. Mobile-first وRTL أصلًا.
//
// كلّ حالة غير عادية لها شكل صريح هنا: «بانتظار تفعيل قاعدة البيانات» ليست
// «لا تملك صلاحية»، وليست شاشة فارغة. هذا الفصل مقصود: خلطهما سبق أن أرسل
// المالك يبحث عن ترحيلة ناقصة بينما كانت المشكلة صلاحية، والعكس.
// ════════════════════════════════════════════════════════════════════════════
import { useCallback, useEffect, useRef, useState, type ReactNode } from "react";
import { type OpsState } from "@/lib/portal/opsCenter";

export const card = "bg-stone-900 border border-stone-800 rounded-xl";

/** زرّ بمساحة لمس ≥44px — يُستعمل على الموقع بيد واحدة وأحيانًا بقفّاز. */
export const tapBtn =
...
/** الشاشة التي يراها المالك حين يدفع الكود قبل تشغيل الـSQL. ليست خطأ. */
  ...
  return (
    <div className={`${card} p-5 text-center space-y-3`} role="alert">
      ...
      <p className="text-xs text-stone-500 leading-6">
        صلاحية إدارة التشغيل تُمنح بمفتاح صريح (operations.manage) من شاشة الصلاحيات.
      </p>
    </div>
  );
}
 ...
 * المهلة تمنع «دوران أبديّ» على شبكة موقع تصوير ضعيفة.
 */
export function useOpsLoad<T>(
  fn: () => Promise<OpsState<T>>,
  deps: unknown[],
  timeoutMs = 20000,
): { st: OpsState<T> | null; reload: () => void } {
  const [st, setSt] = useState<OpsState<T> | null>(null);
  const seq = useRef(0);
    ...
    try {
      const r = await Promise.race([
        ...
        new Promise<never>((_, rej) => {
          timer = setTimeout(() => rej(new Error("ops_timeout")), timeoutMs);
        }),
      ]);
      ...
      setSt({
        state: "error",
        message: e instanceof Error && e.message === "ops_timeout"
...
/** عدّاد قابل للنقر — القلب الحقيقيّ للوحة الجوّال. */
export function Counter({
  ...
  return (
    ...
    >
      <div className={`text-xl font-semibold ${col}`}>{value}</div>
      <div className="text-[11px] text-stone-400 mt-1 leading-4">{label}</div>
    </button>
  );
}

/** شريط جاهزية — النسبة مشتقّة من الخادم، لا تُحسب هنا كي لا تنحرف. */
export function ReadinessBar({ score }: { score: number }) {
  const s = Math.max(0, Math.min(100, Math.round(score)));
  const bg = s >= 90 ? "bg-emerald-600" : s >= 60 ? "bg-amber-600" : "bg-red-700";
  return (
...
export function Flash({ text, tone }: { text: string; tone: "ok" | "bad" }) {
  ...
  return (
    <p
      role="status"
      className={`text-sm rounded-lg px-3 py-2 ${tone === "ok" ? "bg-emerald-950/50 text-emerald-300" : "bg-red-950/50 text-red-300"}`}
    >
      {text}
    </p>
  );
