...
export async function sendWhatsAppTemplate(p: WaTemplateSend): Promise<WaSendResult> {
  ...
  const n8nUrl = process.env.N8N_WHATSAPP_SEND_WEBHOOK_URL;
  if (n8nUrl) {
    try {
      const res = await fetch(n8nUrl, {
        method: "POST",
        headers: { "Content-Type": "application/json", ...(process.env.N8N_WHATSAPP_SEND_SECRET ? { "x-kian-send-secret": process.env.N8N_WHATSAPP_SEND_SECRET } : {}) },
        body: JSON.stringify({
      ...
      return { ok: false, status: "failed", provider: "n8n", messageId: null, error: String((e as Error)?.message ?? e).slice(0, 300) };
    }
  }
  ...
  if (!phoneId || !token) return { ok: false, status: "skipped", provider: null, messageId: null, error: "whatsapp_provider_missing" };
  ...
  try {
    ...
    return { ok: false, status: "failed", provider: "meta_cloud", messageId: null, error: String((e as Error)?.message ?? e).slice(0, 300) };
  }
}
