...
async function request<T>(path: string, opts: RequestOpts, allowRetry = true): Promise<Result<T>> {
  ...
  if (!res.ok) {
    ...
    // تشخيص Dev فقط (يُحذف من حزمة Production عبر NODE_ENV): أظهر خطأ PostgREST الكامل —
    // SQLSTATE/message/details/hint — بينما يبقى ما يراه المستخدم رسالةً عامة موجزة.
    // لا نكشف أي تفاصيل PostgreSQL في متصفح Production ولا في سجلّاته.
