  ...
  if v_enabled then
    ...
    select coalesce(
      ...
      from (
        ...
        -- 🔴 Wave 4 · V2-4.2-D — شرطان لا شرط واحد.
        -- `status='approved'` قرار **داخليّ** بجودة النصّ. و`consent` إذن
        -- **العميل** بالنشر باسمه. اعتمادٌ بلا موافقة يعني نشر اسم عميل لم
        -- يأذن — وهو ما تمنعه القاعدة صراحةً، فأُضيف الشرط الثاني.
        -- (الإدخال الإداريّ يجب أن يضبط consent=true عمدًا، وهو إقرار بامتلاك الإذن.)
