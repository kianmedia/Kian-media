...
from (values
  ...
  ('custody_incidents',       'فرع في hse_register_v — غير محروس')
) v(n, why);

-- ⚠️ **تنبيه تشغيليّ:** Preview أثبت أنّ `custody_incidents` **غير موجود**.
--    ⇒ هذه الحزمة محجوبة حتّى يوجد الجدول، وهذا الفحص سيوقفها صراحةً بدل أن
--    تفشل داخل المعاملة بعد بدء التطبيق.
