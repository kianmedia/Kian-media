...
> ### ⚠️ إضافة بعد قراءة `MASTER_BRIEF.md` (v2.0) — تعارض في عُرف الترحيل
> ضابط **G2** في v2.0 ينصّ على أن تغييرات المخطط تكون «migration files in
> `supabase/migrations/` with a `-- ROLLBACK:` section».
> **الواقع المُقاس:** `supabase/migrations/` **فارغ (0 ملف)**، والعُرف الفعلي القائم هو
> **292 ملفًا في `docs/`** بنمط رباعي ناضج: `PREFLIGHT` → `RUNME` → `POSTCHECK` →
> `ROLLBACK` (ملف تراجع **منفصل** لا قسم داخلي — 24 ملف `*_ROLLBACK.sql`).
>
> **الأثر:** تطبيق G2 حرفيًا يُنشئ **نظام ترحيل ثانيًا موازيًا** لنظام قائم بـ292 ملفًا —
> وهو بالضبط ما تمنعه قاعدة G13. المسودة v2.1 أعادت صياغة G2 لاعتماد العُرف القائم،
> **لكن القرار النهائي لخالد** (سؤال GATE A). راجع
> [`OVERLAP_DEDUP_MATRIX.md`](OVERLAP_DEDUP_MATRIX.md) صف `V2-G.2`.
