...
**FINAL_HEAD:** لا يمكن لالتزامٍ أن يحمل بصمته هو — تثبيتُها يُغيّرها. فالمرجع
الوحيد الصادق هو الأمر نفسه:

```bash
git rev-parse HEAD
```

قارِنها بما في التقرير النهائيّ، وبعد الدفع يجب أن يطابقها `git rev-parse origin/main`.
آخر التزامٍ ذي محتوًى قبل تثبيت هذا السطر: `545908e`.
