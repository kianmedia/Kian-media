...
begin
  ...
  -- (11-ز) ★ ومَن يدخل UNIT_COLS يجب أن يكون عدّادًا **بلفظه** ★
  --   الشبكة الرابعة تُهزم بخطوة واحدة: يكفي أن يُضاف مبلغٌ باسم بريء إلى
  --   UNIT_COLS فيصير «عدّاد وحدات» بإعلانٍ لا بحقيقة — hidden_fee، handling
  --   _figure، settlement_value. المطابقة هنا **إثبات لا نفي**: كلّ اسم في
  --   قائمة العدّادات يجب أن يحمل مفردة عدٍّ صريحة. والسلسلة الجزئية آمنة في
  --   هذا الموضع وحده لأنّ نطاقها ثلاثة أسماء أعلنّاها نحن، لا كلّ أعمدة
  --   الجدول — وهذا هو الفرق عن قائمة المنع التي أسقطت الترحيلة.
  select coalesce(string_agg(u, ' · ' order by u), '') into v_bad
    from unnest(UNIT_COLS) u
   where u !~ '(^|_)(units?|credits?|quantity|qty|count|hours?|days?|sessions?)(_|$)';
  if v_bad <> '' then
    raise exception 'CSUB §17 SELF-TEST: عمود ماليّ في طلبات الخدمة — اسم في UNIT_COLS ليس عدّادًا بلفظه: % — لا يُعلَن مبلغٌ عدّادًا ليعبر الشبكة الرقمية', v_bad;
  end if;
