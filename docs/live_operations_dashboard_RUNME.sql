...
-- ★★ لا COMMIT هنا ★★ كان الملفّ معاملتين: الأولى تُنشئ كلّ شيء وتُثبّته،
--   والثانية تُشغّل الفحص الذاتيّ. وذلك يقلب وظيفة الفحص رأسًا على عقب:
--   الفحص الذاتيّ يوجد ليمنع الترحيلة قبل COMMIT، فإذا سبقه COMMIT صار
--   تقريرًا بعد الوقوع لا مانعًا — وفشلُه يترك اثني عشر جدولًا وستًّا
--   وخمسين دالّة مطبَّقةً بلا مصادقة.
--   ولا ضرورة تقنية تمنع الدمج: لا CONCURRENTLY ولا VACUUM ولا
--   CREATE EXTENSION ولا ALTER TYPE ADD VALUE في أيّ من الكتلتين
--   (المعاملة الثانية لا تُنشئ كائنًا واحدًا أصلًا). فدُمجتا.
...
-- ════════════════════════════════════════════════════════════════════════════
-- (تكملة المعاملة نفسها — الفحص الذاتيّ يسبق COMMIT الوحيد)
do $st$
