...
### `wave7_global_search_*` (١١)

| البند | القيمة |
|---|---|
| **الغرض** | بحث شامل عبر Postgres FTS — ⛔ بلا خدمة خارجية |
| **الاعتماديات** | `projects` (إلزاميّ) · `clients`/`deliverables`/`custody_inventory_assets` (اختيارية — تُتخطّى إن غابت) |
| **العلم** | `NEXT_PUBLIC_SHOW_GLOBAL_SEARCH` (OFF) |
| **مستقلّة؟** | ✅ نعم |
| **نسخة احتياطية؟** | 🟢 لا — دوالّ وفهارس فقط، ⛔ ولا تغيير شكل أيّ جدول |

⚠️ **الكلفة الحقيقية: بناء الفهارس.** أربعة فهارس GIN تُبنى على كامل الجداول
عند التطبيق. اقرأ `ROW_COUNT` في PREFLIGHT أوّلًا، وطبّق في نافذة هادئة.

### `wave7_audit_viewer_*` (١٢)

| البند | القيمة |
|---|---|
| **الغرض** | عارض **قراءة فقط** لـ`activity_log` + سجلّ إحالة لبقيّة سجلّات الوحدات |
| **الاعتماديات** | `activity_log` · `can_manage_projects()` |
| **العلم** | `NEXT_PUBLIC_SHOW_AUDIT_VIEWER` (OFF) |
| **مستقلّة؟** | ✅ نعم |
| **نسخة احتياطية؟** | 🟢 **لا** — دالّتان قراءة فقط، ولا جدول ولا كتابة |
| **الخطر** | 🟢 الأدنى في البرنامج كلّه: لا كتابة إطلاقًا |

🔴 **قرار W7-1 معلَّق:** العارض يقرأ `activity_log` وحده عمدًا. توحيد الخمسة عشر
سجلًّا يُنتج عرضًا يبدو كاملًا وهو ليس كذلك.
