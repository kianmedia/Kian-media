...
### ⚠️ إضافة بعد قراءة `MASTER_BRIEF.md` (v2.0) — نطاق G7 أوسع من Zoho
نصّ G7 في v2.0 هو: «**no AI features; no WhatsApp sending/automation; no Zoho API calls**».
**الثلاثة مبنية ومشحونة في المستودع** — انظر §2 من
[`EXISTING_CAPABILITIES.md`](EXISTING_CAPABILITIES.md) (الوحدات 24 · 32 · 33). ⇒ صياغة
«المنع» غير قابلة للتطبيق، و**النص المعتمد أعلاه يسري حرفيًا على الثلاثة معًا**:
**AI Assistant** و**WhatsApp** و**Zoho**.

**ما يبقى ممنوعًا فعلًا بموجب v2.0 (وهذه ليست موجودة):** مدفوعات إلكترونية · نفاذ ·
مشغّل مراجعة دقيق الإطار (محجوب على قرار Stream vs Mux) · **أتمتة WhatsApp جديدة** ·
**نظام AI ثانٍ**.

❓ **تأكيد GATE A مطلوب على إعادة الصياغة** — راجع `V2-G.7` في
[`OVERLAP_DEDUP_MATRIX.md`](OVERLAP_DEDUP_MATRIX.md).
...
| القيد | الواقع |
|---|---|
| **Cron** | **3 مهام فقط** في `vercel.json` (03:00 / 03:10 / 03:20). خطة Hobby ⇒ ❌ لا background jobs إضافية. أي جدولة جديدة **تُضاف داخل هذه الثلاث أو عبر n8n**. ⚠️ **G8 في v2.0 يقول «no Vercel cron» — والثلاثة قائمة وتعمل** (تنبيهات العهدة · البريد · مزامنة Zoho). الصياغة الصحيحة: **«لا cron إضافي»**؛ إيقاف القائم يعطّل ثلاثة مسارات إنتاجية. ❓ تأكيد GATE A. |
| **الترويسات الأمنية** | مبنية وحيّة في `next.config.js:14-60` — بما فيها CSP بنصفين (منفَّذ + Report-Only) وتعليل موثَّق لـ`geolocation=(self)` (قيمة `()` كانت ستوقف كل تسجيلات حضور الموظفين). ⚠️ **`HSTS` غير مدرج.** ❌ لا يُعاد بناؤها |
| **CI** | `.github/workflows/ci.yml` قائم: `web` (lint→typecheck→test→build) + `mobile` (typecheck + expo-doctor)، على `push` و`pull_request`. ❌ لا يُنشأ workflow فحص ثانٍ — تُضاف خطوات للقائم |
