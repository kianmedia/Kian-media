...
| مجموع عضويات الفئات (مجموع عدّادات الفلاتر) | **54** ✅ (صُحِّح في Wave 1 — كان مكتوبًا 56 خطأً) | عدّ `cats: [...]` على البيانات فقط |
| توزيع الفئات | corporate 12 · events 9 · weddings 7 · commercial 7 · realestate 6 · documentary 6 · cinematic 4 · festivals 3 = **54** | مُثبَّت في `tests/wave1_portfolio_content.test.js` |
| أعمال بوصف فريد (`dAr`) | **8 فقط** | grep |
| أعمال ترث وصف الفئة (مكرر) | **38** | 46 − 8 |
| أعمال باسم «إعلان قصير» | **3** — أسطر 109 / 111 / 113 | `grep -n 'إعلان قصير'` |

> ⚠️ **تصحيح (Wave 1):** الرقم الصحيح **54**، والفحص الخارجي كان محقًّا. رقم 56 في نسخة التدقيق كان خطأً مني — عددتُ سطر تعليق داخل `ITEMS` يحوي `cats: ["corporate", "realestate"]` وكأنه بيانات، فأضاف عضويتين وهميتين.
>
> **تفسير فجوة 54 مقابل 46:** ليست خطأ بيانات. عنصر واحد يمكن أن ينتمي لعدة فئات
