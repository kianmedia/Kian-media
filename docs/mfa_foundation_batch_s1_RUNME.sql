...
--
--   ✅ **زرّ الطوارئ الرسمي والوحيد** — أمر واحد، أثره فوري على الطلب التالي:
--
--        update public.mfa_settings set enforcement_mode = 'off' where id = 1;
--
--   يُنفَّذ من محرّر SQL في Supabase — مسار اعتماد **مستقلّ تمامًا** عن تسجيل الدخول
--   للبوابة وعن Vercel، فلا يمنعه إقفالٌ في البوابة. وكل مسارات MFA تقرأ هذا الوضع
--   **أوّلًا** قبل أيّ شرط آخر، فلا شيء يتجاوزه.
--
--   ⚠️ **حذف العامل ليس وسيلة الخروج.**
--        delete from auth.mfa_factors where user_id = '<uuid>';
--   هذا **إجراء إداري رسمي** لجهاز مفقود فعلًا: يُلغي تسجيل المستخدم ويستوجب إعادة
--   التسجيل من الصفر، ويجب أن يكون موثَّقًا ومُراجَعًا. لا تستعمله لمجرّد تجاوز الشاشة —
--   استعمل `off` أعلاه.
-- ════════════════════════════════════════════════════════════════════════════
