...
-- الترتيب: (1) Operational  (2) RPC signatures + Availability  (3) Client linking
--          (4) Renter self-service + Identity + Consent + Evidence + Reminders
--          (5) Damage settlement + Auto-invoice  (6) Request evidence upload fix.
-- كل جزء idempotent · غير هدّام · لا يحذف بيانات · لا يعيد Foundation · بلا Fixtures.
-- كل جزء يحوي preflight خاصًا + begin/commit + NOTIFY pgrst + Validation.
-- بعد التشغيل: فعّل الأعلام rental_insurance_enabled + rental_customer_portal_enabled
--   (+ rental_finance_enabled فقط بعد اجتياز اختبار المالية/الفواتير). WhatsApp يبقى معطّلًا.
-- ملاحظة: أُنشئ آليًا بدمج الملفات المفردة؛ التعديل يتم على الملفات المصدر ثم يُعاد الدمج.
-- ════════════════════════════════════════════════════════════════════════════
