...
| P1.1 إغلاق ثغرة حقن الطابور | ✅ `75fa3b5` — **SQL لم يُطبَّق (`M-005`)** |
| **P1.2 صحّة تسليم العامل** | ⏭️ **التالي** |
| P1.3 إيقاف تقرير «أخضر» أثناء انقطاع كامل | ⏳ |
| P1.4 إزالة مسارات التجاوز والازدواج | ⏳ |
| P1.5 استعادة نسبة الحدث في المراقبة والتتبّع | ⏳ |
| P1.6 صدق النجاح في النماذج العامة | ⏳ |

### تفصيل P1.2 (ابدأ من هنا)
1. `lib/server/notifyWorker.ts:71` — احتسب المحاولة **عند الحجز** لا عند اكتمالها،
   واحذف الزيادة من `:85` و`:106`. (اليوم: فقدان استجابة الـPATCH يترك الصفّ
   `processing` بنفس عدد المحاولات ⇒ يُعيده الـReaper إلى `pending` بلا احتساب ⇒
   **لا يصل أبدًا إلى `dead_letter`** ويتكرّر يوميًا.)
2. `:84` — لا تَعُدّ `out.sent++` إن فشل الـPATCH (`AdminResult.ok === false`).
3. ارفع الـReaper (`:160-161`) **فوق** فرع `deliveryIds` (`:127`) ليعمل في المسارين.
4. `lib/server/projectNotify.ts:112` — أضِف `AbortSignal.timeout(20_000)`.
5. صدّر `RECOVERY_WINDOW_HOURS` واستخدمه في مسار «Expire old» بالإدارة.
