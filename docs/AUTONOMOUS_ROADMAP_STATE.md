...
| **P2** | Privileged Account MFA (TOTP) | `IN_PROGRESS` | **S3.5 CODE FIXED — OWNER RETEST REQUIRED** | السبب الجذري: `GET /auth/v1/factors` **غير موجود في GoTrue إطلاقًا** ⇒ 405 بجسم فارغ ⇒ رسالة عامة. أُصلح إلى `GET /auth/v1/user` + `data.factors` | **توقّف** — بانتظار اختبار المالك | `23356a6` | ⏸️ `M-000` | ❌ لم يُختبر بعد | 989/989 | `M-003` اختبار المالك | 2026-07-28 |
