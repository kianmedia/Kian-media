...
**الخطوة اليدوية المطلوبة:** لا يوجد أمر سطريّ — المسار الحقيقيّ هو **شاشة الاستيراد**
داخل البوابة: تبويب «استيراد» في صفحة المشروع ← اختر الملفّ ← **Preview** ← **Dry Run**
← **Confirm**. العدد الفعلي للمراحل والمخرجات يُقرأ من الملفّ نفسه ولا يُجبَر على 11/79.

> صحّحتُ هنا ادّعاءً كتبتُه سابقًا في هذا الملفّ عن أمر `npm run misbar:build-payload`.
> **لا وجود لهذا الأمر**، ومحرّك الاستيراد مكتوب بـTypeScript فلا يستطيع سكربت Node
> عاديّ استيراده مباشرة. الشاشة هي الطريق.
...
| 1 — التدقيق وتحليل Excel | 🟡 التدقيق منجز · **Excel غير موجود** | `ee20056` |
| 2 — الاستيراد الجماعي العامّ | ✅ منجز | `919e5ca` + `caa4ebe` |
| 3 — قدرات المشاريع المعقّدة | ✅ منجز | `0368e99` |
| 4 — Dataset القبول | 🟡 **بنية** مسبار جاهزة · مخرجات **اصطناعية** | `ee20056` |
| 5 — حزمة Go-Live | ✅ منجز | `022b0a7` + حزمة التنفيذ |

**Batch 4 ناقص عمدًا:** المراحل الـ11 حقيقية (منك)، والمخرجات اصطناعية معلَّمة
`"synthetic": true`. لن يكتمل قبل وضع `docs/input/MISBAR10_PLAN.xlsx`.
