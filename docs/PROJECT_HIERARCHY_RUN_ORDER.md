...
| 2 | `docs/project_hierarchy_security_RUNME.sql` | **2 — RLS & Access** | `client_project_access` table + `pc_client_cap` capability oracle + visibility-aware redefinition of `can_access_project`/`is_client_side`/`is_client_owner` (no-op for standalone; blocks client access to unauthorized subprojects via RLS across all dependent tables) + admin grant/revoke RPCs + `pc_project_client_caps` read. Flag stays OFF. | **Yes** |
