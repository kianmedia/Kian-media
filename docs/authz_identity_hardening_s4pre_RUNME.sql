...
--
--   ⚠️ **تصحيح لاستشهادي أنا** (اكتشفه تدقيق لاحق): استشهدتُ أدناه بـ
--     `permission_catalog_RUNME.sql:219` و`:258` كموضعَي «إعادة كتابة الصلاحيات».
--     **غير دقيق** — السطران داخل `emp_has_permission` و`emp_can`، وهما **قراءة**
--     لا كتابة. مواضع الكتابة الحقيقية سبعة، مذكورة في
--     `docs/authz_fixB_identity_permissions_RUNME.sql`. الثغرة نفسها حقيقية،
--     لكن موضعها كان مغلوطًا؛ إصلاح دالّة قراءة كان سيترك الكتابة مفتوحة.
--
--   ✓ الثغرة الثانية مؤكَّدة (بالمواضع المصحَّحة):
--     دوال الصلاحيات تقبل
