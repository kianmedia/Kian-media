...
| V2-1.4-D | اشتقاق عدّادات الفئات من البيانات (يقتل 54/46) | ✅ **المجموع الفعلي 54** — والفحص الخارجي كان محقًّا (صُحِّح في Wave 1؛ رقم 56 هنا كان خطأ عدّ: سطر تعليق يحوي `cats: [...]` حُسب بيانات). والفجوة **بالتصميم**: العمل ينتمي لعدة فئات عمدًا | عدّ `cats: [...]` على البيانات → 54 | Medium | 🔧 `VERIFY & EXTEND` + ❓ | ❌ الاشتقاق وحده **لا يزيل الفجوة** — 54 ستبقى 54. القرار: عرض كما هو / توضيح / منع العضوية المتعددة | متوسط — **بند v2.0 مبني على تشخيص ناقص** |
