// SEO landing route — service pages, ar locale.  Wave 1 · V2-1.11
//
...
export function generateStaticParams() {
  ...
  return seoPagesOfKind("service").map((p) => ({ slug: p.slug }));
}

export function generateMetadata({ params }: Params): Metadata {
  const page = seoPageBySlug("service", params.slug);
  ...
  return {
    ...
    alternates: {
      canonical: canonicalFor(seoPagePath(page, "ar")),
      languages: {
    ...
    openGraph: {
      title: ar ? page.titleAr : page.titleEn,
      description: ar ? page.descriptionAr : page.descriptionEn,
      url: canonicalFor(seoPagePath(page, "ar")),
      type: "website",
...
export default function Page({ params }: Params) {
  ...
  const page = seoPageBySlug("service", params.slug);
  if (!page) notFound();
  return <SeoLandingPage page={page} locale="ar" />;
}
